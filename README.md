...
**Backend** | Java 21 · Spring Boot · Spring Security · Spring Data JPA · PostgreSQL · Redis · JWT · WebSocket/STOMP · Spring Actuator  
...
### 4-3. 운영 문제 추적을 위한 로깅 구조화

운영 환경에서 문제가 발생했을 때 요청 흐름과 실패 원인을 빠르게 파악할 수 있도록 로그를 구조화했습니다.

**API 요청 처리 로그 (LoggingFilter)**

모든 HTTP 요청에 대해 메서드, 요청 경로, 응답 상태, 처리 시간을 key-value 형태로 기록했습니다.  
이를 통해 특정 API의 실패 여부나 응답 지연을 로그 검색만으로 빠르게 확인할 수 있도록 했습니다.  
또한 `try-finally` 구조로 구현해 요청 처리 중 예외가 발생하더라도 요청 로그가 남도록 했습니다.

```java
log.info("http_request method={} path={} status={} duration_ms={}", method, uri, status, duration);
```

**에러 로그 구조화 (GlobalExceptionHandler)**

예외의 성격에 따라 로그 레벨을 구분했습니다.
- 예상 가능한 비즈니스 예외 (권한 없음, 존재하지 않는 리소스 등) → `warn`
- 예상하지 못한 서버 오류 → `error`
...
요청 로그와 에러 로그를 모두 key-value 형태로 통일해 특정 코드나 경로로 필터링하기 쉽도록 했습니다.  
예를 들어 잘못된 요청 경로(`/members/NaN/role`)처럼 프론트에서 잘못 전달된 값도 path 로그를 통해 빠르게 확인할 수 있습니다.

### 4-4. 조회 방향을 고려한 인덱스 설계

`ChannelMember`와 `WorkspaceMember` 테이블에는 각각 `(channel_id, user_id)`,  
`(workspace_id, user_id)` 조합으로 UniqueConstraint를 설정했습니다.

이 제약 조건을 통해 중복 멤버 등록을 방지할 수 있고, PostgreSQL에서는 이를 위한 복합 인덱스가 생성됩니다.
...
다만 복합 인덱스는 선두 컬럼 기준 조회에는 효과적이지만,  
선두 컬럼이 아닌 `user_id`만으로 조회하는 경우에는 인덱스를 효율적으로 사용하기 어렵습니다.

예를 들어 다음과 같은 조회는 기존 복합 인덱스만으로는 `user_id` 기준 접근에 한계가 있습니다.
- 이 사용자가 속한 채널 목록 조회
- 이 사용자가 속한 워크스페이스 목록 조회

따라서 사용자 기준 조회 성능을 개선하기 위해 `user_id` 단방향 인덱스를 추가했습니다.
...
@Table(
    ...
    uniqueConstraints = @UniqueConstraint(columnNames = {"channel_id", "user_id"}),
    indexes = @Index(name = "idx_channel_member_user_id", columnList = "user_id")
)
// WorkspaceMember
@Table(
...
인덱스 적용 여부는 `EXPLAIN ANALYZE`로 확인했습니다.

| 테이블 | 개선 전 | 개선 후 |
|---|---|---|
| channel_members | Seq Scan (전체 스캔) | Index Scan |
| workspace_members | Seq Scan (전체 스캔) | Index Scan |

단순히 UniqueConstraint가 있으니 충분하다고 판단하지 않고,  
실제 API의 조회 방향이 `채널/워크스페이스 기준`인지 `사용자 기준`인지 확인한 뒤 필요한 인덱스를 추가했습니다.
...
워크스페이스 목록의 unread 조회 쿼리도 동일한 구조 문제가 있었습니다.
PUBLIC/PRIVATE 채널 멤버 여부를 별도 EXISTS 서브쿼리로 중복 확인하고 있었는데,
워크스페이스는 자신이 속한 워크스페이스만 조회되므로
그 안의 채널도 자신이 멤버인 채널만 관련 있다는 점을 고려해
중복 EXISTS를 제거하고 JOIN 조건만으로 처리하도록 개선했습니다.
...
- 현재 단일 인스턴스 구조이기 때문에 WebSocket 세션과 메시지 전달이 하나의 서버 안에서 처리됩니다.  
  추후 인스턴스가 여러 대로 확장될 경우 서버 간 메시지 전달이 필요하므로, Redis Pub/Sub을 활용해 수평 확장 가능한 구조로 개선하고 싶습니다.
...
Health Check: `https://www.pulsespace.kr/actuator/health`
